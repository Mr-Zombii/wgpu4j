 ...
 */
public class RenderPipelineDescriptor implements Marshalable {
    ...
    public MemorySegment marshal(Arena arena) {
        ...
        MemorySegment primitiveStateStruct = primitiveState.marshal(arena);
        ...
        if (depthStencilState != null) {
            MemorySegment depthStencilPtr = depthStencilState.marshal(arena);
        ...
        MemorySegment multisampleStateStruct = multisampleState.marshal(arena);
    ...
    private void setupVertexState(MemorySegment vertexState, Arena arena) {
        ...
        if (!vertexBuffers.isEmpty()) {
            ...
            for (int i = 0; i < vertexBuffers.size(); i++) {
                MemorySegment bufferLayout = vertexBuffers.get(i).marshal(arena);
    ...
    private void setupFragmentState(MemorySegment fragmentState, Arena arena) {
        ...
        if (!colorTargets.isEmpty()) {
            ...
            for (int i = 0; i < colorTargets.size(); i++) {
                MemorySegment colorTargetStruct = colorTargets.get(i).marshal(arena);
